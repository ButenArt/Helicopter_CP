...
Helicopter_SPU_Bort_Teh_TCP_Client::Helicopter_SPU_Bort_Teh_TCP_Client(Helicopter_core* Helicopter_)
{
    Helicopter = Helicopter_;

    hostaddr = Helicopter->_ini->IP_Bort_Teh();
    tcp_port = Helicopter->_ini->PORT_Bort_Teh();
    ...
    qDebug() << "Connect Bort Teh to " << hostaddr << ":" << tcp_port;
    ...
    connect(timer, &QTimer::timeout, this, &Helicopter_SPU_Bort_Teh_TCP_Client::RitaOFFSlot);
...
Helicopter_SPU_Bort_Teh_TCP_Client::~Helicopter_SPU_Bort_Teh_TCP_Client()
{
}

void Helicopter_SPU_Bort_Teh_TCP_Client::RitaOFFSlot() {
    SendSpuBortTehDataPacket(0);
    flag_H = false;
    timer->stop();
}

void Helicopter_SPU_Bort_Teh_TCP_Client::On_CheckParams()
{
    if (Helicopter->ep_elp.spu.t_radio) {
        if (Helicopter->ep_elp.spu.t_dr) {
            if (Helicopter->ccp.KN53_pozuv_110) {
                if (!flag_H) {
                    SendSpuBortTehDataPacket(60);// 110.5 частота
                    timer->start(9000);
                    flag_H = true;
                }
            }
                ...
                if (!flag_H) {
                    SendSpuBortTehDataPacket(61);// 111.7 частота
                    timer->start(7000);
                    flag_H = true;
                }
            }
                ...
                if (!flag_H) {
                    SendSpuBortTehDataPacket(62);// 113.4 частота
                    timer->start(9000);
                    flag_H = true;
                }
            }
        ...
        else if (Helicopter->ep_elp.spu.t_rk1) {
            if (Helicopter->rsp_elp.ark_9.flag_pomeh) {
                if (!flag_H) {
                    SendSpuBortTehDataPacket(59); // шум
                    timer->start(2000);
                    flag_H = true;
                }
            }
            if (Helicopter->rsp_elp.ark_9.ritaHleb) {
                if (!flag_H) {
                    SendSpuBortTehDataPacket(57); // Хлебодаровка
                    timer->start(8000);
                    flag_H = true;
                }
            }
                ...
                if (!flag_H) {
                    SendSpuBortTehDataPacket(58); // Актобе
                    timer->start(6000);
                    flag_H = true;
                }
            }
        ...
        else if (Helicopter->ep_elp.spu.t_rk2) {
            if (Helicopter->rsp_elp.ark_9.flag_pomeh) {
                if (!flag_H) {
                    SendSpuBortTehDataPacket(59); // шум
                    timer->start(2000);
...
void Helicopter_SPU_Bort_Teh_TCP_Client::Start()
{
    TimerTCPSend->start( );
}

void Helicopter_SPU_Bort_Teh_TCP_Client::Stop()
{
    TimerTCPSend->stop( );
}

void Helicopter_SPU_Bort_Teh_TCP_Client::On_Connected()
{
    qDebug() << "SPU Bort Teh Connected";
}

void Helicopter_SPU_Bort_Teh_TCP_Client::On_Disconnected()
{
    qDebug() << "SPU Bort Teh Disconnected";
}

void Helicopter_SPU_Bort_Teh_TCP_Client::On_Error()
{
}

void Helicopter_SPU_Bort_Teh_TCP_Client::SendSpuBortTehDataPacket(const int track_number)
{
