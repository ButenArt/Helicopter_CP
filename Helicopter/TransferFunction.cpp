...
void TransferFunction::calc_matrix()
{
    ...
    size_t n_states = A.size1();
    size_t n_inputs = B.size2();
    //size_t n_outputs = C.size2();
    int n_steps = 1;
...
void TransferFunction::print(QString name, matrix<double>& M)
{
    ...
    for (size_t j = 0; j < size2 * (width + 2); j++) { std::cout << "_"; }
    std::cout << "_\n";
    for (size_t i = 0; i < size1; i++)
    {
        ...
        std::cout << "|\n";
        for (size_t j = 0; j < size2; j++) { std::cout << "|___________"; }
        std::cout << "|\n";
    }
    std::cout << "\n";
}
...
matrix<double> TransferFunction::expm(const matrix<double> H, double t, const int p)
{
    ...
    c(0) = 1;
    for (size_type i = 0; i < static_cast<size_t>(p); ++i)
    ...
    size_t s = 0; real_value_type scale = 1.0;
    if (norm > 0.5)
    {
