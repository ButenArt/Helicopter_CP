...
HRESULT ControllerInput::AddNewControllers()
{
    HRESULT hr_ = E_FAIL;
    ...
    if (FAILED(hr_ = CreateNewControllers(devicesInfo_)))
        return hr_;
...
HRESULT ControllerInput::PopulateDeviceDescriptions(DEVICE_INFO_VECTOR& devicesInfo)
{
    ...
    for (UINT ii = 0; ii < LogitechControllerInput::LG_MAX_CONTROLLERS; ii++)
    {
        ...
        if (VID_LOGITECH == deviceInfo_.vid &&
            ...
            || PID_F310_GAMEPAD == deviceInfo_.pid
...
HRESULT ControllerInput::CreateNewControllers(CONST DEVICE_INFO_VECTOR& devicesInfo)
{
    HRESULT hr_;
